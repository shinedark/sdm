      ...
      <div className="max-w-6xl w-full">
        ...
        <div className="mb-6 bg-gray-800 rounded-lg p-4">
          ...
          <div className="flex space-x-4">
            <button onClick={() => handleTempoChange(TEMPO_PRESETS.low)} className={`px-6 py-3 rounded-lg font-semibold transition-colors ${globalTempo === TEMPO_PRESETS.low ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
              Low ({TEMPO_PRESETS.low} BPM)
            </button>
            <button onClick={() => handleTempoChange(TEMPO_PRESETS.mid)} className={`px-6 py-3 rounded-lg font-semibold transition-colors ${globalTempo === TEMPO_PRESETS.mid ? 'bg-green-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
              Mid ({TEMPO_PRESETS.mid} BPM)
            </button>
            <button onClick={() => handleTempoChange(TEMPO_PRESETS.high)} className={`px-6 py-3 rounded-lg font-semibold transition-colors ${globalTempo === TEMPO_PRESETS.high ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
        ...
        <StepSequencer patterns={midiPatterns} isPlaying={isPlaying} currentBeat={currentBeat} onPatternSelect={handlePatternSelect} onStepToggle={handleStepToggle} onPlay={handlePlay} onStop={handleStop} onVolumeChange={handleVolumeChange} stepsPerPattern={16} maxSlots={8} className="w-full" globalTempo={globalTempo} mutedPatterns={mutedPatterns} />
      </div>
    </div>;
}