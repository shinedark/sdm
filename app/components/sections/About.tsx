...
const AboutCard: React.FC<{
  ...
  return <div className="flex flex-col md:flex-row bg-white rounded-xl p-6 mb-8 mt-20 mb-20 border border-gray-200 shadow-lg items-center md:items-stretch">
      ...
      <div className="flex-1 flex flex-col justify-center md:pr-8">
        ...
        {about.link && <a href={about.link} target="_blank" rel="noopener noreferrer" className="inline-block bg-gray-900 hover:bg-gray-800 text-white px-6 py-2 rounded-full transition-colors duration-300 mt-2">
            Visit {about.title}
          </a>}
      </div>
      {/* Right: Image or Video */}
      <div className="w-full md:w-64 flex-shrink-0 flex justify-center items-center mt-6 md:mt-0">
        {about.videoUrl ? !showSecondVideo ? <video src={about.videoUrl} controls className="rounded-lg w-full h-40 object-cover shadow-md" poster={about.imageUrl} onEnded={handleVideoEnd} /> : about.secondaryVideoUrl ? <video src={about.secondaryVideoUrl} controls autoPlay className="rounded-lg w-full h-40 object-cover shadow-md" poster={about.imageUrl} /> : null : about.imageUrl ? <img src={about.imageUrl} alt={about.title} className="rounded-lg w-full h-40 object-cover shadow-md" loading="lazy" /> : null}
      </div>
    </div>;
};
const AboutSection: React.FC = () => {
  return <div className="min-h-screen bg-white p-8 mb-20 mt-20 pl-[10vw] md:pl-0">
      <div className="max-w-4xl mx-auto ">
        ...
        <div className="space-y-8 mt-20 mb-20">
          {abouts.map((about, index) => <AboutCard key={index} about={about} />)}
        </div>
      </div>
    </div>;
};
