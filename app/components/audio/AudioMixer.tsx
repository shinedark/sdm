...
const AudioMixer: React.FC<AudioMixerProps> = ({
  ...
  return <div className={`audio-mixer p-6 border rounded-lg bg-gray-900 text-white ${className}`}>
      ...
      <div className="mb-6 p-4 bg-gray-800 rounded">
        ...
        <div className="flex items-center space-x-6">
          <div className="flex-1">
            ...
            <input type="range" min="0" max="1" step="0.01" value={masterVolume} onChange={e => handleMasterVolumeChange(parseFloat(e.target.value))} className="w-full" />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {channels.map(channel => <div key={channel.id} className="channel-strip p-4 bg-gray-800 rounded">
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-medium text-sm">{channel.name}</h4>
              <div className="flex space-x-1">
                <button onClick={() => handleChannelMute(channel.id, !channel.mute)} className={`px-2 py-1 rounded text-xs ${channel.mute ? 'bg-red-600' : 'bg-gray-600'}`}>
                  M
                </button>
                <button onClick={() => handleChannelSolo(channel.id, !channel.solo)} className={`px-2 py-1 rounded text-xs ${channel.solo ? 'bg-yellow-600' : 'bg-gray-600'}`}>
                  S
                </button>
              </div>
            </div>
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                <input type="range" min="0" max="1" step="0.01" value={channel.volume} onChange={e => handleChannelVolumeChange(channel.id, parseFloat(e.target.value))} className="w-full h-32 -rotate-90 origin-center" style={{
            ...
            <div className="mb-4">
              <label className="block text-xs font-medium mb-2">
                Pan: {channel.pan > 0 ? `R${(channel.pan * 100).toFixed(0)}` : channel.pan < 0 ? `L${Math.abs(channel.pan * 100).toFixed(0)}` : 'C'}
              </label>
              <input type="range" min="-1" max="1" step="0.01" value={channel.pan} onChange={e => handleChannelPanChange(channel.id, parseFloat(e.target.value))} className="w-full" />
            </div>

            {/* Effects */}
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <input type="checkbox" checked={channel.effects.reverb.enabled} onChange={e => onChannelChange?.(channel.id, {
              effects: {
                ...channel.effects,
                reverb: {
              ...
              <div className="flex items-center space-x-2">
                <input type="checkbox" checked={channel.effects.delay.enabled} onChange={e => onChannelChange?.(channel.id, {
              effects: {
                ...channel.effects,
                delay: {
                  ...channel.effects.delay,
                  enabled: e.target.checked
                }
              }
            })} className="rounded" />
              ...
              <div className="flex items-center space-x-2">
                <input type="checkbox" checked={channel.effects.filter.enabled} onChange={e => onChannelChange?.(channel.id, {
              effects: {
                ...channel.effects,
                filter: {
                  ...channel.effects.filter,
                  enabled: e.target.checked
                }
              }
            })} className="rounded" />
